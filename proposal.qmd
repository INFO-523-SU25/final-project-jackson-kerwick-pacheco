...
# Traffic data sourced from https://www.fhwa.dot.gov/policyinformation/tables/tmasdata/ with aggregate station data over all months from 2019 to 2023. 
# FWHA changed their data format in 2022, so the data are stored in two different formats

traffic_year = {}

for year in np.arange(2019,2024):
  if (year < 2022):   # old format
    ...
    df = df[(df['State_Code'] == 4)].values
  else:
    # Traffic Data New Format
    df = pd.read_csv('data/Traffic/AZ_2023 (TMAS).txt', sep='|')

  traffic_year[year] = df
  print(f"Data from {year}:\n{df}")
